...
1. Last-Modified/If-Modified-Since if-none-modified
...
判断该对象是否经历过Scavenge算法的回收。
判断新生代的To空间占用是否超过25%。

遍历老生代所有对象，将存活的对象打上标记
回收未标记的对象
当老生代的碎片空间不足以为从新生代晋升来的对象分配内存时，执行标记整理，清除内存碎片。
